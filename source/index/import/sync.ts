...
function calcLabelDifference(
  ...
  newLabels.forEach((newLabel: Octokit.IssuesGetLabelResponse):
    | number
    | void => {
    ...
    const matchedLabelDescription =
      (matchedLabel.description && matchedLabel.description.trim()) || '';
    const newLabelDescription =
      (newLabel.description && newLabel.description.trim()) || '';

    // if we have a match, but properties are not equal
    if (
      matchedLabel.color !== newLabel.color ||
      matchedLabelDescription !== newLabelDescription
    ) {
  ...
  const unMutualLabels: Octokit.IssuesGetLabelResponse[] = currentLabels.filter(
    (currentLabel: Octokit.IssuesGetLabelResponse) =>
      mutualLabels.indexOf(currentLabel) === -1
  );
...
export async function syncRepositoryLabels({
  ...
  try {
    ...
    fetchSpinner.start();
    // get all labels
    const oldLabels: Octokit.IssuesGetLabelResponse[] = await apiClient.getLabels(
      destRepo
    );
    const newLabels: Octokit.IssuesGetLabelResponse[] = await apiClient.getLabels(
      sourceRepo
    );

    // calculate the differences
    const labelDiff: DiffEntryProperties[] = calcLabelDifference(
      oldLabels,
      newLabels
    ).filter((diff: DiffEntryProperties): boolean => {
    ...
    fetchSpinner.succeed(
      `Fetched ${newLabels.length} labels from source repository`
    );
    fetchSpinner.start(`Syncing GitHub issue labels. Please wait...`);

    // perform create/update/delete actions
    const syncStatus: SyncActionResultProps[] = await syncLabelAction(
      apiClient,
      labelDiff,
      destRepo
    );
    ...
    let updated = 0;
    let deleted = 0;
    const successCount: number = syncStatus.filter(
      ({type, status}: SyncActionResultProps): boolean => {
        if (status) {
          if (type === 'missing') {
            created += 1;
        ...
        return (
          status &&
          (type === 'missing' || type === 'updatable' || type === 'deletable')
        );
      }
    ).length;

    fetchSpinner.succeed(
      `Sync complete. ${successCount} successful, ${
        labelDiff.length - successCount
      } failed.`
    );
    fetchSpinner.succeed(
